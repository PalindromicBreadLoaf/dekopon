...
SwitchFrontend::MotionState ReadSevenSixAxis() {
    ...
    // libnx has not named these fields yet; Horizon lays out acceleration, angular velocity,
    // then the orientation quaternion in the ten floats starting at offset 0x18.
    return {
        .active = true,
...
SwitchFrontend::MotionState PollMotion(PadState& pad) {
    ...
    // Some compatible Joy-Cons leave the Handheld motion data empty. Automatic mode can use the
    // console sensor because it follows the same physical movement, while explicit left/right
    // choices must keep forcing the controller handle. Firmware without the seven-six-axis sensor
    // falls through to the handheld handle below.
