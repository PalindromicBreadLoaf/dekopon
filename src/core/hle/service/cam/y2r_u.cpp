...
void Y2R_U::StartConversion(Kernel::HLERequestContext& ctx) {
    ...
    // Software video decoders repeatedly use Y2R while a movie is playing. Three conversions
    // separated by no more than 300 ms are enough to start the experimental throttle.
    // 500 ms without another conversion ends it.
    ...
    if (movie_detect_burst_count >= kMovieDetectBurstThreshold) {
        system.SetMoviePlaybackState(Core::System::MoviePlaybackSource::Y2r, true);
    }
...
Y2R_U::Y2R_U(Core::System& system) : ServiceFramework("y2r:u", 1), system(system) {
    ...
    movie_detect_stop_event =
        system.CoreTiming().RegisterEvent("Y2R Movie Detect Stop Event", [this](uintptr_t, s64) {
            movie_detect_last_call_tick = 0;
            movie_detect_burst_count = 0;
            this->system.SetMoviePlaybackState(Core::System::MoviePlaybackSource::Y2r, false);
        });
}
