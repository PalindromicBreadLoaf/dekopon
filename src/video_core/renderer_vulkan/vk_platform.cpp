...
vk::UniqueInstance CreateInstance(const Common::DynamicLibrary& library,
    ...
    // Sanitize layers list. Skipped when nothing was requested, as NXVK is reached through a bare
    // ICD entrypoint that need not expose the global layer-enumeration function.
    if (!layers.empty()) {
